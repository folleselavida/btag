<?php foreach($coment as $com):?>
	<div class="coments">
		<div class="perfil noBorder">
			<img src="/images/tags/<?php echo $com["calificacion"]?>s.png" alt=" Calificación a <?php echo $com["nombre"]?>" class="tag2" />
			<a href="<?php echo url_for("sitio", array("nombre"=>Iridian::slugify($com["nombre"]), "id"=>$com["id"]))?>" class="img">
				<img src="/uploads/sitio/<?php echo $com["thumbnail"]?>" alt=" Review de <?php echo $com["nombre"]?>"  />
			</a>
			
			<p>
				<?php echo $com["nombre"]?> <br/>
				<?php echo "COUNT"?> Reviews
			</p>
		</div>
		<div class="text_review2">
		<a href="<?php echo url_for("sitio", array("nombre"=>Iridian::slugify($com["nombre"]), "id"=>$com["id"]))?>" class="title1">
				<?php echo $com["nombre"]?>
			</a>
			<h1>
				<?php echo $com["direccion"]?>
			</h1>
			<h1>
				<?php echo $com["telefono"]?>
			</h1>
			<p>
				<?php echo html_entity_decode(substr($com["descripcion"], 0,150))?>
				<a href="">...Leer Más.</a
			></p>
		</div>
	</div>	
<?php endforeach;?>	

<a id="ver_com" class="ver_mas" href="">ver mas</a>